...
const deleteProduct = (req, res) => {
  ...
  validateConnection()
    .then((connection) => {
      ...
      connection.query(updateQuery, [id_product], (err, result) => {
        connection.release();
        ...
        if (err) {
          ...
          res.status(500).json({
            status_Code: 500,
            statusMessage: "Error al intentar actualizar el estado del producto",
            error: err.message,
          });
        } else if (result.affectedRows === 0) {
          res.status(404).json({
            status_Code: 404,
            statusMessage: "No se encontró el producto con el ID proporcionado",
          });
        } else {
          res.status(200).json({
            status_Code: 200,
            statusMessage: "Estado del producto actualizado con éxito",
          });
        }
      });
    })
